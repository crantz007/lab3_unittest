...
class PhoneAssignments():
    ...
    def add_employee(self, employee):
        ...
        for employeeSameID in self.employees:
            if employee.id == employeeSameID.id:
                raise PhoneError(' Same ID found ! cannot be added')

        self.employees.append(employee)

    def add_phone(self, phone):
        # TODO raise exception if two phones with same ID are added
        for phoneSameID in self.phones:
            if phone.id == phoneSameID.id:
                raise PhoneError('Phone with same ID found! cannot be added')
    ...
    def assign(self, phone_id, employee):
        ...
        for phone in self.phones:
            if phone.id == phone_id and phone.employee_id != None:
                raise PhoneError
        # return if employee already has the phone
        for phone in self.phones:
            if phone.employee_id == employee.id:
                raise PhoneError
        # exception for employee already having a phone
        for phone in self.phones:
    ...
    def phone_info(self, employee):
        ...
        if employee not in self.employees:
            raise PhoneError('Employee does not exists')
